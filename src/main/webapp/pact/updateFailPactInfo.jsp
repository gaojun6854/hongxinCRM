					...
					.on(
							...
							function() {
								...
								var index = layer
										.open({
											type : 2,
											content : 'pactInfo!findCheckReceipt.action?pactId=${pactInfo.pactId}',
											area : [ '330px', '210px' ],
											maxmin : true
										});
								layer.full(index);
				...
				<div class="row-fluid form-wrapper">
					...
					<div class="span9 with-sidebar">
						<div class="container">
							...
							<div class="field-box">
								<label>合同状态</label>
								<c:if test="${pactInfo.pactFlow=='2'}">
									<input class="span5 inline-input" type="text" value="初审失败"
										style="color: red" />
								</c:if>
								<c:if test="${pactInfo.pactFlow=='4'}">
									<input class="span5 inline-input" type="text" value="复审失败"
										style="color: red" />
								</c:if>
							</div>
							<div class="field-box">
								...
								<c:if test="${pactInfo.pactFlow=='2'}">
									<input class="span5 inline-input" type="text"
										value="${pactInfo.noPassReson}" />
								</c:if>
								<c:if test="${pactInfo.pactFlow=='4'}">
									<input class="span5 inline-input" type="text"
										value="${pactInfo.lastNoPassReson}" />
								</c:if>
							</div>
							<div class="span11 field-box actions">
								<input type="submit" class="btn-glow primary" value="保存并退回初审" />
